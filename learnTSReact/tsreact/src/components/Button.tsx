...
type Props = {
	...
	// onClick: (e: React.MouseEvent) => void // this time, the parameter is an 'event'; in this case, a React event. The event is typed as a 'MouseEvent'
	onClick: (e: React.MouseEvent<HTMLButtonElement>) => void // same as above, but this more specifically identifies the event as a button click from a HTMLButtonElement, using the "<HTMLButtonElement>" generic type. NOTE that for TS generics, all of the properties of the generic being passed in between the "<>" are accessible by the parent. e.g. Array<string> means that all the properties of a string are now available to the Array type.
